  ...
  wrapWithBoilerplate(code) {
    ...
    let capStr = this.indent(
      Object.keys(this.caps)
        .map(k => {
          return `desiredCapabilities.setCapability(${JSON.stringify(
            k
...
`;
  ...
  codeFor_findAndAssign(strategy, locator, localVar, isArray) {
    ...
    if (isArray) {
      return `List<MobileElement> ${localVar} = (MobileElement) driver.findElementsBy${
        suffixMap[strategy]
      }(${JSON.stringify(locator)});`;
    } else {
      return `MobileElement ${localVar} = (MobileElement) driver.findElementBy${
        suffixMap[strategy]
      }(${JSON.stringify(locator)});`;
    }
  }
  ...
  codeFor_sendKeys(varName, varIndex, text) {
    return `${this.getVarName(varName, varIndex)}.sendKeys(${JSON.stringify(
      text
    )});`;
  }
